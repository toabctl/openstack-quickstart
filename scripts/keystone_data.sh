...
ADMIN_TENANT=$(openstack project create -c id -f value admin)
SERVICE_TENANT=$(openstack project create -c id -f value $SERVICE_TENANT_NAME)
DEMO_TENANT=$(openstack project create -c id -f value demo)
INVIS_TENANT=$(openstack project create -c id -f value invisible_to_admin)
...
if [[ "$ENABLED_SERVICES" =~ "n-cpu" ]]; then
    ...
    if [[ "$KEYSTONE_CATALOG_BACKEND" = 'sql' ]]; then
        ...
        NOVA_V3_SERVICE=$(openstack service create -c id -f value \
            --type=computev3 \
    ...
    RESELLER_ROLE=$(openstack role create -c id -f value admin ResellerAdmin)
...
if [[ "$ENABLED_SERVICES" =~ "tempest" ]]; then
    ...
    ALT_DEMO_TENANT=$(openstack project create -c id -f value \
                             alt_demo)
    ALT_DEMO_USER=$(openstack project create -c id -f value \
        --password="$ADMIN_PASSWORD" \
...
if [[ "$ENABLED_SERVICES" =~ "c-api" ]]; then
    ...
    if [[ "$KEYSTONE_CATALOG_BACKEND" = 'sql' ]]; then
        ...
        CINDER_V2_SERVICE=$(openstack service create -f value id -c \
                        --type=volumev2 \
